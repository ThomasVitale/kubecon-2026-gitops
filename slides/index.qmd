...
%%| fig-width: 10
flowchart LR
  user["You"] -->|"thumbs up"| app["App"]
```

::: {.notes}
Start simple. The user taps thumbs up. What happens inside the app?
:::

## The App Creates a Span {data-transition="none"}

```{mermaid}
%%| fig-width: 10
flowchart LR
  user["You"] -->|"thumbs up"| app["App"]
  app --> span["evaluate UserSatisfaction<br/>(span)"]
```

The app creates an **OpenTelemetry span** — a unit of work

::: {.notes}
First mention of OpenTelemetry. A span is a structured unit of work — connected to other spans in a trace. The app creates one called "evaluate UserSatisfaction" every time someone votes.
:::
...
flowchart LR
  ...
  style grp fill:none,stroke:none
```
...
::: {.notes}
Empty diagram — just the four participants. Set the stage.
:::
...
::: {.notes}
Step 1: The server calls the model to generate a story part.
:::
...
::: {.notes}
Step 2: This is wrapped in a storygen span — the activate bar shows the span is open while we wait for the model.
:::
...
::: {.notes}
Step 3: The model responds with the story part. The storygen span closes — it now contains the model name and response ID.
:::
...
::: {.notes}
Step 2: The storygen span wraps the model call. After it closes, the server sends the story plus the spanContext to the phone. The spanContext is the token that will link everything together later.
:::
...
::: {.notes}
Step 3: The user reads the story. The phone is holding onto the spanContext this whole time.
:::
...
::: {.notes}
Step 4: The user taps thumbs up.
:::
...
::: {.notes}
Step 5: The phone sends the vote AND the spanContext back to the server. The spanContext has round-tripped: server to phone and back.
:::
...
::: {.notes}
Step 6: The server creates the evaluate satisfaction span, links it to the storygen span using the spanContext.
:::
...
::: {.notes}
Step 7: The span event with the vote data lives inside the evaluate satisfaction span.
:::
...
::: {.notes}
Step 7: Everything flows to the OTel Collector via OTLP. The spanContext is what links the storygen span to the evaluate satisfaction span.
:::
